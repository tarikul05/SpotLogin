			...
			$.each(result.data, function(key, value) {
				...
				if (value.ready_flag == "0") {
					...
					if (value.event_type == 10 && value.count_student > 1) {
						resultHtml += '<td style="text-align:right" colspan="2">' + value.price_currency + ' ' + value.buy_price_teacher.toFixed(0) + '</td>';
						value ['buy_total'] = value['buy_price_teacher'];
					} else {
						resultHtml += '<td style="text-align:right" colspan="2">' + value.price_currency + ' ' + value.buy_total.toFixed(0) + '</td>';
					}
